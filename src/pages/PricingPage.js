...
const PricingPage = () => {
    ...
    const handleOpenFeaturesModal = async (packageId) => {
        ...
        try {
            const resultAction = await dispatch(fetchPackageFeatures({
    ...
    const handleAddFeature = async (feature) => {
        ...
        const packageItem = pricingData.find(pkg => pkg.package_id === currentPackageId);
        if (!packageItem) return;

        try {
            // 1. Save the new feature
            const saveResult = await dispatch(savePackageFeatures({
                id: 0,
            ...
            if (savePackageFeatures.fulfilled.match(saveResult)) {
                ...
                const refreshResult = await dispatch(fetchPackageFeatures({
                    package_id: currentPackageId,
                    lang_code: packageItem.lang_code
                }));
    ...
    const handleDeleteFeature = async (featureIndex) => {
        ...
        try {
            ...
            const deleteResult = await dispatch(savePackageFeatures({
                id: featureId,
                package_id: currentPackageId,
            ...
            <main className={`main-content ${menuExpanded ? 'menu-expanded' : 'menu-collapsed'}`}>
                ...
                {showFeaturesModal && currentPackageId && (
                    <FeaturesModal
                        ...
                        packageName={pricingData.find(pkg => pkg.package_id === currentPackageId)?.package_name}
