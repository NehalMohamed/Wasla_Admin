    ...
    builder
      ...
      .addCase(saveFeature.fulfilled, (state, action) => {
        ...
        const existingIndex = state.items.findIndex(
          (f) => f.feature_id === action.payload.id
        );
        if (existingIndex >= 0) {
      ...
      .addCase(saveFeatureTranslation.fulfilled, (state, action) => {
        ...
        if (state.currentFeature) {
          const translationIndex =
            state.currentFeature.features_Translations?.findIndex(
          ...
          if (action.payload.delete) {
            ...
            state.currentFeature.features_Translations =
              state.currentFeature.features_Translations?.filter(
                (t) => t.id !== action.payload.id
              );
          } else if (translationIndex >= 0) {
            // Update existing translation
            state.currentFeature.features_Translations[translationIndex] =
              action.payload;
          } else {
            // Add new translation
            state.currentFeature.features_Translations = [
