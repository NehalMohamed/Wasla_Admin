  ...
  const renderProductTree = (products, level = 0) => {
    return products.map((product) => (
      <React.Fragment key={product.productId}>
        <tr className={`product-row ${!product.active ? 'inactive-row' : ''}`} data-active={product.active.toString()}>
          <td style={{ paddingLeft: `${level * 20}px` }}>
            {product.productName}
          </td>
          <td>
            {product.productParent === 0 ? 'Root' : 
              parentProducts.find(p => p.productId === product.productParent)?.productName || 'N/A'}
          </td>
